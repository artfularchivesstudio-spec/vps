// Deno-specific image analysis for Supabase Edge Functions
...
export async function analyzeWithOpenAI(imageUrl: string, prompt?: string, analysisType: string = 'detailed') {
  ...
  const defaultPrompt = ANALYSIS_PROMPTS[analysisType as keyof typeof ANALYSIS_PROMPTS] || ANALYSIS_PROMPTS.detailed;
...
// Claude Analysis - DISABLED: Credit balance too low
...
export async function analyzeImage(request: any) {
  try {
    const { imageUrl, imageData, prompt, providers = ['openai'], analysisType = 'detailed', saveToStorage = false, title } = request;
    ...
    if (providers.includes('openai')) {
      analysisPromises.push(
        analyzeWithOpenAI(finalImageUrl, prompt, analysisType)
          .then((result) => {
    ...
    if (providers.includes('claude')) {
      analysisPromises.push(
        analyzeWithClaude(finalImageUrl, prompt, analysisType)
          .then((result) => {
...
function generateSuggestions(analyses: any[]) {
  const content = analyses.map((analysis: any) => analysis.content).join(' ');
  
  // Extract key themes and elements for suggestions
  const words = content.toLowerCase().split(/\s+/);
  const artTerms = [
    'painting', 'drawing', 'sculpture', 'abstract', 'portrait', 'landscape',
    'still life', 'contemporary', 'modern', 'traditional'
  ];
  
  const foundTerms = words.filter((word: string) => artTerms.includes(word));
...
function extractTags(content: string) {
  const commonTags = [
    'art', 'painting', 'drawing', 'creative', 'visual', 'color',
    'composition', 'style', 'modern', 'contemporary'
  ];
  
  const contentLower = content.toLowerCase();
  return commonTags.filter((tag: string) => contentLower.includes(tag)).slice(0, 5);
}
